...
class _Map3MLP(nn.Module):
    """
    wCDM  : 6 inputs  → [64, 256, 1024, 1024, 256, 192] → N outputs
    LCDM  : 5 inputs  → [64, 256, 1024, 1024, 384, 192] → N outputs
    """
...
class _Map3Network:
    ...
    def predictions_np(self, params_dict: Mapping[str, Sequence[float]]) -> np.ndarray:
        ...
        x = np.array([[params_dict[k][0] for k in keys]], dtype=np.float32)
        with torch.no_grad():
            out = self._model(torch.tensor(x, device=self._device)).cpu().numpy()
...
def _build_parameter_vector(
    ...
    if model_cosmo == "wCDM":
        ...
        return parameters, {k: [0.0] for k in ["Omega_m", "s8", "h0", "Omega_b", "ns", "w"]}
    ...
    return parameters, {k: [0.0] for k in ["Omega_m", "s8", "h0", "Omega_b", "ns"]}
