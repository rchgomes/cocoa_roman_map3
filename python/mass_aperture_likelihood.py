...
def rescale_params(params: np.ndarray, scale: Mapping[str, Mapping[str, float]]) -> np.ndarray:
...
def post_process(array: np.ndarray, scale: Mapping[str, Sequence[float]]) -> np.ndarray:
...
def upsampling(z: np.ndarray, pred: np.ndarray, n: int) -> Tuple[np.ndarray, np.ndarray]:
    if n <= len(z):
        return z, pred
...
def _load_source_distributions(nz_files: Sequence[str]) -> Tuple[List[np.ndarray], List[np.ndarray]]:
...
def _load_map3_metadata(metadata_file: str) -> Tuple[np.ndarray, Dict[str, np.ndarray], int]:
...
def _build_network(model_cosmo: str, modes: np.ndarray) -> cosmopower_NN:
    if model_cosmo == "wCDM":
        network = cosmopower_NN(
            parameters=["Omega_m", "s8", "h0", "Omega_b", "ns", "w"],
            modes=modes,
            n_hidden=[64, 256, 1024, 1024, 256, 192],
        )
    else:
        network = cosmopower_NN(
            parameters=["Omega_m", "s8", "h0", "Omega_b", "ns"],
            modes=modes,
...
def setup_map3_module(
    *,
    ...
    if missing:
        raise ValueError(
            "Missing required mass-aperture configuration option(s): " + ", ".join(missing)
        )
    ...
    modes = np.arange(filter_num * len(z_values))
    network = _build_network(cosmo_model, modes)
...
def _get_sigma8(cosmology_parameters: Mapping[str, float], provider: Any) -> float:
    ...
    if provider is not None:
        try:
            ...
            raise ValueError("sigma8 is required but was not available from the provider.") from exc
    raise ValueError("sigma8 is required in cosmology_parameters or via the Cobaya provider.")
...
def _prediction_dict(params_for_network: np.ndarray, model_cosmo: str) -> Dict[str, List[float]]:
    if model_cosmo == "wCDM":
        return {
            "Omega_m": [params_for_network[0]],
...
def _reshape_predictions(predictions_rescaled: np.ndarray, zarray: np.ndarray, filter_num: int) -> np.ndarray:
...
def _set_linear_theory(bs: Any, provider: Any, cosmology_parameters: Mapping[str, float], zarray: np.ndarray):
    if provider is None:
        raise ValueError(
            "The Cobaya theory provider is required to build the linear matter power and growth inputs."
        )
    ...
    k_growth = 5.0e-4
    growth = np.sqrt(pk_interp.P(z_for_pk, k_growth) / pk_interp.P(0.0, k_growth)) * (1.0 + z_for_pk)
    growth /= growth[-1]
...
def _set_ia_parameters(
    ...
    if perbin:
        ...
        for i in range(nzbin):
            if provider is None:
                raise ValueError(
                    "per-bin IA requires the Cobaya provider so nuisance parameters can be read."
                )
            nla_param[f"AIA_{i + 1}"] = provider.get_param(f"{ia_amplitude_prefix}{i + 1}")
...
def compute_map3(
    ...
    **kwargs: Any,
    ...
    cosmology_parameters
        Mapping with at least ``H0``, ``omegam``, ``omegab``, ``ns``, ``w`` and
        either ``sigma8`` or a Cobaya provider that can supply it.
    data_vector_file, covariance_file
        Unused directly here, but kept in the signature for compatibility with
        ``cosmic_shear_2pt_map3``.
    nz_files
        Sequence of plain-text files with at least two columns: ``z`` and
        ``n(z)``.
    kwargs
    ...
    config = setup_map3_module(
        ...
        cosmo_model=kwargs.get("cosmo_model", "LCDM"),
    ...
    parameters, _ = _build_parameter_vector(
        cosmology_parameters,
        config["model_cosmo"],
        provider,
    )
    params_for_network = rescale_params(parameters, config["rescaling_params"])
    predictions = config["network"].predictions_np(
        _prediction_dict(params_for_network, config["model_cosmo"])
    )
    ...
    nz_paths = [_normalize_path(path) for path in _as_list(nz_files)]
    if not nz_paths:
    ...
    if config["nz_upsampling"] > len(zarray):
        zarray, predictions_newshape = upsampling(zarray, predictions_newshape, config["nz_upsampling"])
